...
void base_sim_t::mouse_down(const b2Vec2 &p)
{
	...
	{
		body1 = callback.m_fixture->GetBody();
		b2MouseJointDef mjointDef;
		body1->SetLinearVelocity(d);
		mjointDef.bodyA = m_ground_body;
		body1->SetGravityScale(0);
		mjointDef.bodyB = body1;
		mjointDef.collideConnected = true;
	  	mjointDef.maxForce = 0.0* body1->GetMass();
		body1->SetTransform(p,body1->GetAngle());
		mjointDef.dampingRatio= 0;
		m_mouseJoint = (b2MouseJoint*)m_world->CreateJoint(&mjointDef);
...
void base_sim_t::shift_mouse_down(const b2Vec2& p)
{
	m_mouseWorld = p;
	body1->SetTransform(p,body1->GetAngle());
	if (m_mouseJoint != NULL)
